...
<input name="gh_lang" value="kr" hidden="">
	<div class="gh-detail">
		<h1>${r.placeName }</h1>
		<div>평점 평균: ${r.gradeAvg } 리뷰 수: ${r.gradeCnt }</div>
		<br>
		<div>도로명 주소: ${r.addressDoro }</div>
		<div>지번 주소: ${r.addressJibun }</div>
		<div>
			카카오맵 URL: <a href="${r.placeUrl }">${r.placeUrl }</a>
		</div>
		<br>
		<!-- ---------------------지도 api------------------------ -->
	...
	<form action="RentShopDetailC" method="post" enctype="multipart/form-data">
		...
		별점 <select name="rentGrade">
			...
			<textarea class="gh-review-car" name ="rentCar" rows="" cols="" maxlength="100"
				placeholder="대여한 차종을 입력해주세요(최대 100자)"></textarea>
			<br>
			<textarea class="gh-review-write" name="rentReview" rows="" cols="" maxlength="500"
				placeholder="후기를 입력해주세요(최대 500자)"></textarea>
			<br>
	...
	<div class="pageInfo">
		<a class="gh-review-page-a" value="1"> [맨 앞] </a>
		<c:forEach begin="1" end="${totalReviewPage }" var="i">
			<a class="gh-review-page-a" value="${i}" >[${i }]</a>
		</c:forEach>
		<a class="gh-review-page-a" value="${totalReviewPage }"> [맨 끝] </a>
	</div>
